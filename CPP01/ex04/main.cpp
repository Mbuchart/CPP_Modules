...
std::string scan_line(std::string line, std::string search, std::string replace)
{
	...
	{
		j = 0;
		while (line[i + j] && search[j] == line[i + j])
			j++;
		if (j == search.length())
		{
...
int main(int ac, char **av)
{
	if (ac != 4)
	{
		std::cerr << "Please launch the program as follow : ./replace file searched_string replace_string" << std::endl;
		return (1);
	}
	...
	{
		std::cout << "couldn't open the file : " << av[1] << std::endl;
	}
	return (0);
}